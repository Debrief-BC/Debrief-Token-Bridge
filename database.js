...
/*
_DBFNetUserWalletAddr       : input user Wallet in DBF chain
_DBFParkingWalletETH        : Parking wallet in Eth network associated to _DBFNetUserWalletAddr
_DBFParkingWalletETHPrivKey : Private key of _DBFParkingWalletETH
 ...
 *
 * @param {string} _DBFNetUserWalletAddr        - Wallet address user entered
 * @param {string} _DBFParkingWalletETH         - Auto generated ETH network key
 * @param {string} _DBFParkingWalletETHPrivKey  - Auto generated ETH network key
 */
let insertIntoDB = function insertIntoDB(
  _DBFNetUserWalletAddr,
  _DBFParkingWalletETH,
  _DBFParkingWalletETHPrivKey
) {
  let returnVal = false;
  try {
    MongoClient.connect(dbconfig.dbURL, { useNewUrlParser: true }, function(
      ...
      db.collection(dbconfig.dbKeysCollection).insertOne({
        DBFNetUserWalletAddr: _DBFNetUserWalletAddr,
        DBFParkingWalletETH: _DBFParkingWalletETH,
        DBFParkingWalletETHPrivKey: _DBFParkingWalletETHPrivKey
      });
...
/**
 * Functin used to search/ retrieve associated wallet mapping in DBF and ETH networks
 * returns the document (eth network to DBF network mapping addresses)
...
let retriveAssociatedWallet = function retriveAssociatedWallet(
  ...
  return new Promise(function(resolve, reject) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var query;
      if (isParkingWallet) query = { DBFParkingWalletETH: _SearchWallet };
      else query = { DBFNetUserWalletAddr: _SearchWallet };
      ...
      db.collection(dbconfig.dbCollectionName).findOne(query, function(
        ...
        if (err) {
          ...
          if (doc) {
            //doc.DBFParkingWalletETHPrivKey ='';
          }
          resolve(doc);
        }
        //client.close();
 ...
 * 2. When DBF coins are sent in mainnet
 * 3. When DBF coins transfer transaction  is confirmed after 'n' blocks
 *
      ...
      db.collection(dbconfig.dbTxCollection).findOneAndUpdate(
        ...
        function(err, doc) {
          ...
          if (err) {
            ...
            //console.log('Debug '+doc.DBFNetUserWalletAddr,' : ',doc.DBFParkingWallet);
            if (doc) {
              //doc.DBFParkingWalletETHPrivKey ='';
            }
            resolve(doc);
 ...
 *
 * @param {*} _DBFParkingWalletETH
 * @param {*} _DBFNetUserWalletAddr
...
/**
 * Functin used to retrieve txs for which we need to send DBFs for delayed token bridge
