...
var debrief_network = new Web3(global.gConfig.target_network);
var coldstorage = global.gConfig.coldstorage_address;
var dbf_contract_eth_network = global.gConfig.token_contract_on_source_network;
...
debrief_network.eth.net
  .isListening()
  .then(() => console.log("Connected to debrief network"))
  .catch(e => {
    console.log("Error!! not connected to debrief network");
    process.exit(1);
  });
...
async function get_nonce(pubkey) {
  nonce = await debrief_network.eth.getTransactionCount(pubkey);
}

//get balance from eth address on target network
async function get_from_address(txhash) {
  obj = await debrief_network.eth.getTransaction(txhash);
  return debrief_network.utils.fromWei(
    await debrief_network.eth.getBalance(obj.from),
    "ether"
  );
}
...
var subscription = etheruem_network.eth
  .subscribe(
    "logs",
    {
      address: dbf_contract_eth_network,
      topics: [
